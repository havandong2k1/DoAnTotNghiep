...
class OrderController extends Controller
{
    ...
    public function view_order($order_code){
        $order_details_p = OrderDetails::with('product')->where('order_code', $order_code)->get();
        ...
        if($product_coupon != 'Không có mã giảm giá'){
            ...
            $coupon_condition = $coupon->coupon_condition; // tính bằng mã giảm giá
            $coupon_number = $coupon->coupon_number;    // tính bằng tiền được giảm
        }else{
            $coupon_condition = 2;
            $coupon_number = 0;
        }

        return view('admin.view_order')->with(compact('order_details_p', 'customer', 'shipping', 'order_details', 'coupon_condition', 'coupon_number','getorder','order_status'));
    ...
    public function print_order_convert($checkout_code){
        ...
        <h3><center>Cộng hòa xã hội chủ nghĩa Việt Nam</center></h1>
    ...
    public function history(Request $request){
        ...
        return view('pages.history.history')->with('category', $cate_product)->with('brand', $brand_product)->with('meta_desc', $meta_desc)->with('meta_keywords', $meta_keywords)->with('meta_title', $meta_title)->with('url_canonical', $url_canonical)->with('slider',$slider)->with('category_post',$category_post)->with('getorder',$getorder);
    ...
    public function view_history_order(Request $request, $order_code){
        ...
        return view('pages.history.view_history_order')->with('category', $cate_product)->with('brand', $brand_product)->with('meta_desc', $meta_desc)->with('meta_keywords', $meta_keywords)->with('meta_title', $meta_title)->with('url_canonical', $url_canonical)->with('slider',$slider)->with('category_post',$category_post)->with('order_details_p',$order_details_p)->with('customer',$customer)->with('shipping',$shipping)->with('order_details',$order_details)->with('coupon_condition',$coupon_condition)->with('coupon_number',$coupon_number)->with('getorder',$getorder)->with('order_status',$order_status);
